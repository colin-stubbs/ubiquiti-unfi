...
tc qdisc add dev switch0.${SWITCH_SUBINT} handle ffff: ingress
tc -s qdisc ls dev switch0.${SWITCH_SUBINT}

# direct traffic out the physical interface connected to the Corelight sensor
tc filter add dev switch0.${SWITCH_SUBINT} parent ffff: protocol all u32 match u32 0 0 action mirred egress mirror dev ${IDS_PORT}
